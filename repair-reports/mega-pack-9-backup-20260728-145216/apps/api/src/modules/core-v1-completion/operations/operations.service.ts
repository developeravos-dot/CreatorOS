...
export class OperationsService {
  ...
  async transitionDeployment(id: string, input: TransitionDeploymentDto) { const found = await this.prisma.opsDeployment.findUnique({ where: { id } }); if (!found) throw new NotFoundException('Deployment not found.'); const terminal: OpsDeploymentStatus[] = [OpsDeploymentStatus.SUCCEEDED, OpsDeploymentStatus.FAILED, OpsDeploymentStatus.ROLLED_BACK]; const row = await this.prisma.opsDeployment.update({ where: { id }, data: { status: input.status, completedAt: input.completedAt ? new Date(input.completedAt) : terminal.includes(input.status) ? new Date() : undefined, notes: input.notes } }); await this.audit('deployment.transitioned', 'OpsDeployment', id, { status: row.status }); return row; }
}
