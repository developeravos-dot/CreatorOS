...
describe('MulticulturalAdaptationStageService', () => {
  let service: MulticulturalAdaptationStageService;
  beforeEach(async () => {
    const module: TestingModule = await Test.createTestingModule({ providers: [MulticulturalAdaptationStageService] }).compile();
    service = module.get(MulticulturalAdaptationStageService);
  });
