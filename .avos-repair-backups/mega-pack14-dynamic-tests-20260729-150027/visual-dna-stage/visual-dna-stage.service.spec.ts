...
describe('VisualDnaStageService', () => {
  let service: VisualDnaStageService;
  beforeEach(async () => {
    const module: TestingModule = await Test.createTestingModule({ providers: [VisualDnaStageService] }).compile();
    service = module.get(VisualDnaStageService);
  });
