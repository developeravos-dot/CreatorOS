...
describe('RoyaltyAccountingStageService', () => {
  let service: RoyaltyAccountingStageService;

  beforeEach(async () => {
    const module: TestingModule = await Test.createTestingModule({ providers: [RoyaltyAccountingStageService] }).compile();
    service = module.get<RoyaltyAccountingStageService>(RoyaltyAccountingStageService);
  });
