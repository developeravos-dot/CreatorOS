import {
  Test,
  TestingModule,
} from '@nestjs/testing';

import {
  MarketOpportunityAnalysisStageController,
} from './market-opportunity-analysis-stage.controller';

import {
  MarketOpportunityAnalysisStageService,
} from './market-opportunity-analysis-stage.service';

describe('MarketOpportunityAnalysisStageController', () => {
  let controller: MarketOpportunityAnalysisStageController;

  beforeEach(async () => {
    const module: TestingModule =
      await Test.createTestingModule({
        controllers: [
          MarketOpportunityAnalysisStageController,
        ],
        providers: [
          MarketOpportunityAnalysisStageService,
        ],
      }).compile();

    controller =
      module.get<MarketOpportunityAnalysisStageController>(
        MarketOpportunityAnalysisStageController,
      );
  });

  it('should be defined', () => {
    expect(controller).toBeDefined();
  });

  it('should expose operational dashboard', () => {
    expect(
      controller.getDashboard().status,
    ).toBe('operational');
  });

  it('should expose 24-stage blueprint', () => {
    expect(
      controller.getBlueprint().stages,
    ).toHaveLength(24);
  });
});
