...
describe('BrandDesignSystemStageService', () => {
  let service: BrandDesignSystemStageService;
  beforeEach(async () => {
    const module: TestingModule = await Test.createTestingModule({ providers: [BrandDesignSystemStageService] }).compile();
    service = module.get(BrandDesignSystemStageService);
  });
