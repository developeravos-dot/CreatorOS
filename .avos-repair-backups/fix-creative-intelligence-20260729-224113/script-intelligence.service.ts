  ...
  build(
    ...
    return {
      ...
      scenes: Array.from({ length: sceneCount }, (_, index) => {
        const act = story.acts[Math.min(story.acts.length - 1, Math.floor(index / Math.ceil(sceneCount / 3)))];

        return {
          id: randomUUID(),
          order: index + 1,
          heading: `SCENE ${index + 1} — ${act.name.toUpperCase()}`,
          purpose: index === 0 ? 'hook' : index === sceneCount - 1 ? 'resolution' : act.purpose,
          dialogue: [
      ...
      languageVersions: (brief.languages ?? ['Arabic', 'English']).map((language) => ({
        language,
        status: 'planned',
      })),
    };
