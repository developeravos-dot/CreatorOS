...
export class IntegrationsService {
  ...
  async transitionDelivery(id:string,input:TransitionDeliveryDto){
    ...
    const isCurrentTerminal =
      found.status === IntegrationDeliveryStatus.DELIVERED ||
      found.status === IntegrationDeliveryStatus.DEAD_LETTER ||
      found.status === IntegrationDeliveryStatus.CANCELLED;
    if (isCurrentTerminal && input.status !== found.status) {
      throw new ConflictException('A terminal delivery cannot be transitioned.');
    }
  ...
  async transitionSaga(id:string,input:TransitionSagaDto){
    const found=await this.prisma.integrationSaga.findUnique({where:{id}});
    if(!found) throw new NotFoundException('Integration saga not found.');
    const isCurrentTerminal =
      found.status === IntegrationSagaStatus.COMPLETED ||
      found.status === IntegrationSagaStatus.FAILED ||
      found.status === IntegrationSagaStatus.COMPENSATED ||
      found.status === IntegrationSagaStatus.CANCELLED;
    if(isCurrentTerminal) throw new ConflictException('Saga is already terminal.');
    const isNextTerminal =
      input.status === IntegrationSagaStatus.COMPLETED ||
      input.status === IntegrationSagaStatus.FAILED ||
      input.status === IntegrationSagaStatus.COMPENSATED ||
      input.status === IntegrationSagaStatus.CANCELLED;
    const row=await this.prisma.integrationSaga.update({where:{id},data:{status:input.status,state: input.state === undefined ? undefined : (input.state as any), error: input.error,completedAt:isNextTerminal?new Date():undefined}});
    await this.audit('saga.transitioned','IntegrationSaga',id,{status:row.status});
    return row;
  }
}
