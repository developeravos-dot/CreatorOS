...
export class IntegrationsService {
  ...
  async createEndpoint(input: CreateIntegrationEndpointDto) { try { const row=await this.prisma.integrationEndpoint.create({ data: input }); await this.audit('endpoint.created','IntegrationEndpoint',row.id,{ endpointKey: row.endpointKey }); return row; } catch(e){ this.conflict(e); } }
  async updateEndpoint(id:string,input:UpdateIntegrationEndpointDto){ const found=await this.prisma.integrationEndpoint.findUnique({where:{id}}); if(!found) throw new NotFoundException('Integration endpoint not found.'); const row=await this.prisma.integrationEndpoint.update({where:{id},data:input}); await this.audit('endpoint.updated','IntegrationEndpoint',id,{status:row.status}); return row; }

  listEventDefinitions(){ return this.prisma.integrationEventDefinition.findMany({orderBy:[{domain:'asc'},{eventName:'asc'}]}); }
  async createEventDefinition(input:CreateEventDefinitionDto){ try { const row=await this.prisma.integrationEventDefinition.create({data:input}); await this.audit('event-definition.created','IntegrationEventDefinition',row.id,{eventName:row.eventName}); return row; } catch(e){this.conflict(e);} }
  ...
  async publishEvent(input:PublishIntegrationEventDto){
    ...
    const event=await this.prisma.integrationEvent.create({data:{eventName:input.eventName,aggregateType:input.aggregateType,aggregateId:input.aggregateId,payload:input.payload,correlationId:input.correlationId}});
    const hooks=await this.prisma.integrationWebhook.findMany({where:{active:true}});
    const matching=hooks.filter((h: { eventNames: unknown })=>Array.isArray(h.eventNames) && (h.eventNames as unknown[]).includes(input.eventName));
    if(matching.length){ await this.prisma.integrationDelivery.createMany({data:matching.map((h: { id: string })=>({eventId:event.id,webhookId:h.id,status:'PENDING'}))}); }
  ...
  async createSaga(input:CreateSagaDto){ try { const row=await this.prisma.integrationSaga.create({data:{...input,status:'RUNNING',startedAt:new Date()}}); await this.audit('saga.created','IntegrationSaga',row.id,{sagaKey:row.sagaKey}); return row; } catch(e){this.conflict(e);} }
  async transitionSaga(id:string,input:TransitionSagaDto){ const found=await this.prisma.integrationSaga.findUnique({where:{id}}); if(!found) throw new NotFoundException('Integration saga not found.'); const terminal:IntegrationSagaStatus[]=[IntegrationSagaStatus.COMPLETED,IntegrationSagaStatus.FAILED,IntegrationSagaStatus.COMPENSATED,IntegrationSagaStatus.CANCELLED]; if(terminal.includes(found.status)) throw new ConflictException('Saga is already terminal.'); const row=await this.prisma.integrationSaga.update({where:{id},data:{status:input.status,state:input.state,error:input.error,completedAt:terminal.includes(input.status)?new Date():undefined}}); await this.audit('saga.transitioned','IntegrationSaga',id,{status:row.status}); return row; }
}
