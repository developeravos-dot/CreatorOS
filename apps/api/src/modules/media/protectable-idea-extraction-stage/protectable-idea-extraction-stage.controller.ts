...
@Controller('media/ip-empire/protectable-idea-extraction')
export class ProtectableIdeaExtractionStageController {
  constructor(
    private readonly service: ProtectableIdeaExtractionStageService,
      private readonly duplicateAnalyzer: ProtectableIdeaDuplicateAnalyzerService,
  ) {}
  ...
  async analyzeIdea(
    ...
    const originalityResult =
      await this.service.analyzeIdea(input);

    const duplicateAnalysis =
      this.duplicateAnalyzer.analyzeCriticResult(
        originalityResult,
        input.content,
      );

    if (
      typeof originalityResult === 'object' &&
      originalityResult !== null &&
      !Array.isArray(originalityResult)
    ) {
      return {
        ...originalityResult,
        duplicateAnalysis,
  ...
  analyzeDuplicates(
    ...
    return this.duplicateAnalyzer.analyzeCriticResult(
