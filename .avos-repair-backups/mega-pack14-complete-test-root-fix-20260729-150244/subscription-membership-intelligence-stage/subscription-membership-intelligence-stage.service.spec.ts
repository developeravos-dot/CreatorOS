...
describe('SubscriptionMembershipIntelligenceStageService', () => {
  let service: SubscriptionMembershipIntelligenceStageService;

  beforeEach(async () => {
    const module: TestingModule = await Test.createTestingModule({ providers: [SubscriptionMembershipIntelligenceStageService] }).compile();
    service = module.get<SubscriptionMembershipIntelligenceStageService>(SubscriptionMembershipIntelligenceStageService);
  });

  it('should be operational', () => {
    const dashboard = service.getDashboard();
    expect(dashboard.status).toBe('operational');
    expect(dashboard.totalStages).toBe(36);
    expect(dashboard.humanFinalAuthority).toBe(true);
  });

  it('should expose the complete blueprint', () => {
    expect(service.getBlueprint().stages).toHaveLength(36);
  });
  ...
  it('should create a portfolio', () => {
    ...
    expect(portfolio.stages).toHaveLength(36);
  });
