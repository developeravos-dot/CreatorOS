...
describe('CreativeExperimentationStageService', () => {
  let service: CreativeExperimentationStageService;
  beforeEach(async () => {
    const module: TestingModule = await Test.createTestingModule({ providers: [CreativeExperimentationStageService] }).compile();
    service = module.get(CreativeExperimentationStageService);
  });
